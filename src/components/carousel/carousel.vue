<template>
  <div :class="classes">
    <button type="button" :class="arrowClasses" class="left" @click="arrowEvent(-1)">
      <b-icon name="ios-arrow-back"></b-icon>
    </button>
    <div :class="[prefixCls + '-list']">
      <div :class="[prefixCls + '-track', showCopyTrack ? '' : 'higher']" :style="trackStyles" ref="originTrack">
        <slot></slot>
      </div>
      <div :class="[prefixCls + '-track', showCopyTrack ? 'higher' : '']" :style="copyTrackStyles" ref="copyTrack"
           v-if="loop">
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    add(offset) {
      ...
      this.$emit('change', oldIndex, this.currentIndex)
